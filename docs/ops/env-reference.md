...
| `REVORA_MODEL` / `REVORA_VISION_MODEL` / `REVORA_REASONING_EFFORT` / `REVORA_DAILY_CHECK_CAP` | **temporary — delete on sight after the Vercel rename** | Pre-rename names, still what Vercel production actually carries. Read only when the matching `PAL_*` is unset, so this branch could merge without a lockstep env change. ⛔ Without the `REVORA_MODEL` fallback an unset `PAL_MODEL` resolves to the unprefixed default while `OPENAI_BASE_URL` still targets OpenRouter, and `assertModelIdMatchesTransport` throws on every `/api/check` — an outage, not a quiet downgrade. Removal steps and the full file list: `docs/ops/outstanding.md` §1. |
...
| `LONGITUDINAL_INSIGHTS_ENABLED` | owner/evidence gate | **Server twin / runtime kill switch** for `NEXT_PUBLIC_LONGITUDINAL_INSIGHTS` (`lib/longitudinal-insights-flag.ts`): only exact `1` permits server-side insight output. Same twin-guard rule as the photo pair |
...
| `NEXT_PUBLIC_APP_URL` | 4D/P7 | canonical origin — `https://revora.plus` in production. Never `revora.app`: that domain belongs to an unrelated company |
...
| ⚙ `AUTH_EMAIL_FROM` | P5 | envelope sender override, e.g. `Revora <signin@send.revora.plus>` for preview. Defaults to `Revora <signin@contact.revora.plus>` (`lib/pal/contact.ts`) — `contact.revora.plus` is the domain verified in Resend (2026-07-21). The sending domain must be verified in Resend with DKIM published, or every magic link silently fails SPF/DKIM alignment and lands in spam — which on a passwordless product is a login outage, not a deliverability nit |
