...
```
- Create: `components/home-door.tsx` — client wrapper `<HomeDoor ideas={…} quickRow={…} step={…} hero={…} />` that renders the default order on the server (`ideas, hero, quickRow, step` — A-54) and after `useHydrated()` *(final review 2026-09-18: shipped as `useSyncExternalStore`, not `useHydrated()` — the react-hooks `set-state-in-effect`, `refs` and `purity` lint rules are errors in this repo)* reorders per `doorFor(askStore.get())`: `numbers` → the step line (step 1 = Learn: numbers) above the ideas block and the ideas eyebrow reads "Ideas for later" (row); `plan` → the step line first, ideas second; `worried` → a one-line clinician pointer (`BOUNDARY_DISCLAIMER`'s second sentence is the approved wording — reuse it, no new claim) above everything, then ideas.
