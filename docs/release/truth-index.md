...
| C5 | Landing "weekly pattern" phrasing (was `app/page.tsx:315`) | Flag-gated "weekly pattern" copy | **CLOSED 2026-08-14.** Two things resolved it. (1) The flag is now owner-authorized ON (`docs/legal/owner-decision-2026-08-14-longitudinal-insights-on.md`), so the "pending flag-on" condition is met. (2) The copy itself is gone — the landing was rebuilt in PR #114 and **no `weekly pattern` string survives anywhere in `app/`, `components/`, or `lib/`**. The nearest live copy is the includes-carousel panel "Learn your patterns." (`app/page.tsx:441`), which promises a browsable history illustrated by the real `/meals` screen and prices nothing. T10's disposition still governs: thin insight is free for all, not a Premium gate. | Product | closed |
