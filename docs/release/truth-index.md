...
| C5 | Landing "weekly pattern" phrasing (was `app/page.tsx:315`) | Flag-gated "weekly pattern" copy | **CLOSED 2026-08-14.** Two things resolved it. (1) The flag is now owner-authorized ON (`docs/legal/owner-decision-2026-08-14-longitudinal-insights-on.md`), so the "pending flag-on" condition is met. (2) The copy itself is gone — the landing was rebuilt in PR #114 and **no `weekly pattern` string survives anywhere in `app/`, `components/`, or `lib/`**. The nearest live copy is the includes-carousel panel titled "Learn your patterns." in `app/page.tsx`'s `includes` array (cited by title — the line moves with the stack), which promises a browsable history illustrated by the real `/meals` screen and prices nothing. T10's disposition still governs: thin insight is free for all, not a Premium gate. | Product | closed |
...
| C7 | Public "wellness tool, not a medical device" assertion (landing, distribution copy) | "Informational-only by design (wellness tool, not a medical device)" | **Overstated — must not stand** (K5, §16). **Removed from product copy — verified 2026-09-06:** no "medical device" / "wellness tool" string remains in `app/`, `components/`, `lib/`, `README.md`, or `docs/product-marketing.md` (case-insensitive scan, now pinned by `tests/unit/pal/device-status-copy-absent.test.ts`); the only remaining referents are the historical counsel-panel review and a protocol checkbox. Counsel must still classify intended use/labeling/each function vs current FDA guidance before any device-status wording is reintroduced anywhere. | Counsel | pending counsel (copy removed) |
...
| `NEXT_PUBLIC_LONGITUDINAL_INSIGHTS` | Client build | Thin longitudinal "weekly pattern" phrasing on landing/dashboard (T10) | **off** (`!== "1"`) | **ENABLED in production** per plan §3.2 line 130 (pre-branch snapshot). **Owner-authorized 2026-08-14** — `docs/legal/owner-decision-2026-08-14-longitudinal-insights-on.md` supersedes the insights bullet in `owner-risk-launch-decision-5f6abcb.md`, which had said the flag must stay unset. Client flag re-verified live that day: `/privacy` renders `derived pattern summaries` + `personalized pattern summaries`, which it emits only at `1`. ⚠️ The server twin was **not** verified live (no unauthenticated probe — `GET /api/coach` 401s before the flag branch); inferred from the `next.config.ts` production twin guard. | RD review of eval-dependent phrasing + product decision on whether thin insight stays free vs replaced by weekly artifact (P2.4 / T10). **T10 disposition governs going forward:** thin insight is free for all; false weekly-insights Premium promises removed — so the enabled prod state renders as free thin insight, not a Premium gate (see C4/C5). |
| `NEXT_PUBLIC_PHOTO_INPUT` | Client build | Photo → draft-chip input path (`lib/photo-input-flag.ts`) | **off** / fail-closed (`!== "1"`) | **ENABLED in production** per plan §3.2 line 130 (pre-branch snapshot); behavior hardened by T4. **Owner-authorized 2026-08-14** — `docs/legal/owner-decision-2026-08-14-photo-assist-on.md` supersedes the photo-assist bullet in `owner-risk-launch-decision-5f6abcb.md`, which had said the flag must stay unset. Re-verified live that day: the route answers `400`, not `404`. | Photo stratum in permanent eval + credentialed review (P1.5); keep confirmation-step explicit. |
