 ...
 * absent.
 *
 * Non-production override seam: set PAL_LAUNCH_MODE_OVERRIDE=paused to
...
function getOverrideMode(): LaunchMode | null {
  ...
  const override = process.env.PAL_LAUNCH_MODE_OVERRIDE?.trim().toLowerCase();
  if (override === "paused") {
    return "paused";
  }
  return null;
 ...
 *
 * Resolution order:
 *  1. Non-production PAL_LAUNCH_MODE_OVERRIDE (test / dev only)
