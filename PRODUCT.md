...
The rule, stated positively so it needs no forbidden words to express: **Prediabetes Pal is never the agent of a health outcome — only the user is.** Any sentence that makes Prediabetes Pal, or the app's use, the thing that achieves an outcome is out of bounds, whatever verb it uses. See `docs/safety/claims-boundary.md` §Banned Claim Families for the enumerated vocabulary; `tests/unit/pal/claims-boundary-copy.test.ts` enforces it across `app/**`, `components/**`, and the fenced regions of this file and the Play listing.
