...
describe("privacy-minimal audit", () => {
  it("keeps one OpenAI wrapper, one route-to-service path, and no raw request logging", () => {
    ...
    const serviceSource = readProjectFile("lib/pal/service.ts");
    const wrapperSource = readProjectFile("lib/pal/openai-client.ts");
    const telemetrySource = readProjectFile("lib/pal/telemetry.ts");
  ...
  it("emits coarse completion telemetry after checkFood returns", async () => {
    ...
    expect(checkFoodImpl).toHaveBeenCalledWith(
      ...
      // here — no x-pal-clarified header). Task 13 §P3.1: plus the snapshot
