 ...
 * client and asserts the EXACT PalUserResponse. Any behavioral drift in
 * lib/pal/ — floors, routing, precheck, fallback copy, disclaimer — fails
 * this suite. Do not "fix" these expectations to make a diff pass; a failure
 * here means lib/pal/ behavior changed, which this build forbids.
 */
...
function scriptedModel(output: PalModelOutput) {
  return { generate: vi.fn().mockResolvedValue(output) };
}

function unsafeSafeOutput(): PalModelOutput {
...
describe("engine regression: golden floor scenarios", () => {
  ...
  it("boundary exactness — 5.7 and 6.4 are in scope; 5.6999 and 6.5 are not", async () => {
    const safeOutput: PalModelOutput = {
      kind: "result",
      risk: "SAFE",
