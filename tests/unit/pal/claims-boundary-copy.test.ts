...
const BANNED: Family[] = [
  ...
  },
  {
    ...
    // describing Prediabetes Pal's users.
    exemptSources: ["app/(app)/how-it-works/page.tsx"]
  },
  // ── New, 2026-07-11 (W-09). Claims, not verbs. ────────────────────────────

  {
    // F-24. Prediabetes Pal has not launched and has zero subscribers. Every one of
...

  {
    ...
    // in terms that it is not a result from Prediabetes Pal's users and not a promise
  ...
  {
    // F-04. lib/pal/postprocess.ts:assertNoUnsafeSafeFields THROWS if a SAFE
...
describe("claims-boundary copy audit", () => {
  ...
  const KNOWN_BAD: Record<string, string[]> = {
    ...
    diagnose: ["Prediabetes Pal can diagnose prediabetes"],
