...
import { EMAIL_FROM, SUPPORT_EMAIL } from "../../../lib/pal/contact";

/**
 * Every address Prediabetes Pal shows a user, or sends mail from, must be on a domain
 * we own.
 *
 ...
 * through lib/pal/contact.ts and this stays true by construction.
 */
...
describe("owned domains", () => {
  ...
  it("has no unowned domain anywhere in shipped source", () => {
    const contact = path.join(ROOT, "lib/pal/contact.ts");
