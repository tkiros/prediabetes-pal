...
describe("get-the-app page store waitlists", () => {
  ...
  it("segments a single shared URL with ?platform= so demand stays measurable", async () => {
    vi.stubEnv("NEXT_PUBLIC_WAITLIST_ANDROID_URL", "");
    vi.stubEnv("NEXT_PUBLIC_WAITLIST_IOS_URL", "");
    vi.stubEnv("NEXT_PUBLIC_WAITLIST_URL", "https://tally.so/r/prediabetespal-waitlist");
    const text = await renderText();
    expect(text).toContain("https://tally.so/r/prediabetespal-waitlist?platform=android");
    expect(text).toContain("https://tally.so/r/prediabetespal-waitlist?platform=ios");
  });
});
